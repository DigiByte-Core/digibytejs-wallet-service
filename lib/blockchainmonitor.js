...
BlockchainMonitor.prototype.start = function(opts, cb) {
  ...
  var self = this;
    ...
    function(done) {
      ...
      async.each(coinNetworkPairs, function(pair, callback) {
        var config = {};
        if (opts.blockchainExplorerOpts && opts.blockchainExplorerOpts && opts.blockchainExplorerOpts[pair.network]) {
          config = opts.blockchainExplorerOpts[pair.network];
        } else {
          return;
        }
        var bcOpts = {
        ...
        var bc = new BlockchainExplorer(bcOpts);
        bc.getBestExplorer(bcOpts, function(err, explorer) {
          if (err) {
            return callback(err);
          }
          $.checkState(explorer);
          self._initExplorer(pair.network, explorer);
          self.explorers[pair.network] = explorer;
        });
      }, function(err) {
        done(err);
      });
...
BlockchainMonitor.prototype._handleIncomingPayments = function(network, data) {
  var self = this;
  ...
  async.each(outs, function(out, next) {
    ...
    self.storage.fetchAddressByCoin(out.address, function(err, address) {
      if (err) {
...
BlockchainMonitor.prototype._handleIncomingTx = function(network, data) {
  ...
  this._handleIncomingPayments(network, data);
};
