...
TxProposal.create = function(opts) {
  ...
  x.addressType = opts.addressType || (x.walletN > 1 ? Constants.SCRIPT_TYPES.P2SH : Constants.SCRIPT_TYPES.P2WPKH);
...
TxProposal.prototype._buildTx = function() {
  ...
  var t = new digibyte.Transaction();
  ...
  _.each(self.outputs, function(o) {
    ...
    if (o.script) {
      t.addOutput(new digibyte.Transaction.Output({
        script: o.script,
        satoshis: o.amount
      }));
    } else {
  ...
  // Validate actual inputs vs outputs independently of digibyte
...
TxProposal.prototype._addSignaturesToBitcoreTx = function(tx, signatures, xpub) {
  ...
  var i = 0,
    x = new digibyte.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = digibyte.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: digibyte.crypto.Signature.SIGHASH_ALL | digibyte.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
