...
Wallet.create = function(opts) {
  ...
  x.addressType = opts.addressType || Constants.SCRIPT_TYPES.P2WSH;
...
Wallet.fromObj = function(obj) {
  ...
  x.addressType = obj.addressType || Constants.SCRIPT_TYPES.P2WSH;
...
Wallet.prototype.createLegacyAddress = function(isChange) {
  $.checkState(this.isComplete());
  ...
  var path = this.addressManager.getNewAddressPath(isChange);
  log.verbose('Deriving addr:' + path);
  var address = Address.derive(self.id, 'P2PKH', this.publicKeyRing, path, this.m, this.network, isChange);
  return address;
};
