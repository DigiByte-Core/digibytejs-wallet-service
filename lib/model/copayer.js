...
Copayer.prototype.createLegacyAddress = function(wallet, isChange) {
  $.checkState(wallet.isComplete());

  var path = this.addressManager.getNewAddressPath(isChange);
  var address = Address.derive(wallet.id, 'P2SH', wallet.publicKeyRing, path, wallet.m, wallet.network, isChange);
  return address;
};
