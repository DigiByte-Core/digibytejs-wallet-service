...
WalletService.getInstanceWithAuth = function(opts, cb) {
  ...
  console.log(opts.copayerId, 'skksks');
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  var addressType = (opts.n == 1 && opts.supportBIP44AndP2PKH) ? Constants.SCRIPT_TYPES.P2PKH : Constants.SCRIPT_TYPES.P2SH;1
...
WalletService.prototype.getWalletFromIdentifier = function(opts, cb) {
  ...
  async.parallel([
    ...
    async.detectSeries(coinNetworkPairs, function(coinNetwork, nextCoinNetwork) {
      var bc = self._getBlockchainExplorer(coinNetwork.network);
...
WalletService.prototype._checkTxInBlockchain = function(txp, cb) {
  ...
  var bc = this._getBlockchainExplorer(txp.network);
  if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
...
WalletService.prototype.broadcastTx = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      console.log(txp.getBitcoreTx().toBuffer(true).toString('hex'));
  ...
  function getNormalizedTxs(wallet, addresses, from, to, cb) {
    ...
    async.series([
      ...
      function(next) {
        ...
        var bc = self._getBlockchainExplorer(wallet.network);
...
WalletService.prototype.scan = function(opts, cb) {
  ...
  function scanBranch(wallet, derivator, cb) {
    ...
    async.whilst(function() {
      ...
      var addressLegacy = derivator.deriveLegacy();
      ...
      checkActivity(wallet, address.address, function(err, activity) {
        ...
        if (!activity)  {
          checkActivity(wallet, addressLegacy.address, function(err, activity) {
            if (err) return next(err);
            allAddresses.push(addressLegacy);
        ...
        self.storage.storeWallet(wallet, function(err) {
          ...
          _.each([false, true], function(isChange) {
            derivators.push({
              ...
              deriveLegacy: _.bind(wallet.createLegacyAddress, wallet, isChange),
              ...
              _.each(wallet.copayers, function(copayer) {
                if (copayer.addressManager) {
                  derivators.push({
                    ...
                    deriveLegacy: _.bind(copayer.createLegacyAddress, copayer, wallet, isChange),
