...
/**
 * Creates an instance of the Digibyte Wallet Service.
 * @constructor
 */
 ...
 * @param {string} opts.network[='livenet'] - The Digibyte network for this wallet.
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new digibyte.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype.getWalletFromIdentifier = function(opts, cb) {
  ...
  async.parallel([
    ...
    function(done) {
      self.storage.fetchAddressByCoin(opts.identifier, function(err, address) {
    ...
    async.detectSeries(coinNetworkPairs, function(coinNetwork, nextCoinNetwork) {
      self._getBlockchainExplorer(coinNetwork.network, function(err, bc) {;
        if (err || !bc) return nextCoinNetwork(false);
        bc.getTransaction(opts.identifier, function(err, tx) {
          ...
          async.detect(toAddresses, function(addressStr, nextAddress) {
            self.storage.fetchAddressByCoin(addressStr, function(err, address) {
...
WalletService.prototype._verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new digibyte.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  var xPubKey;
  try {
    xPubKey = digibyte.HDPublicKey(opts.xPubKey);
  } catch (ex) {
 ...
 * @param {string} opts.unit - Digibyte unit used to format amounts in notifications.
 */
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['dgb', 'bit'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._canCreateAddress = function(ignoreMaxGap, cb) {
  ...
  self.storage.fetchAddresses(self.walletId, function(err, addresses) {
    ...
    self._getBlockchainExplorer(latestAddresses[0].network, function(err, bc) {
      if (err || !bc) return cb(new Error('Could not get blockchain explorer instance'));
...
WalletService.prototype._getBlockchainExplorer = function(network, cb) {
  var opts = {};
  ...
  var bc;
  try {
    var explorer  = new BlockchainExplorer(opts)
    explorer.getBestExplorer(opts, function(err, insightInstance) {
      return cb(null, insightInstance);
    });
  } catch (ex) {
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = digibyte.Address(addresses[0]).toObject().network;

  self._getBlockchainExplorer(networkName, function(err, bc) {
    if (err || !bc) return cb(new Error('Could not get blockchain explorer instance'));
...
WalletService.prototype._getUtxosForCurrentWallet = function(opts, cb) {
  ...
  var allAddresses, allUtxos, utxoIndex, addressStrs;
  ...
  async.series([
    ...
    function(next) {
      ...
      addressStrs =  _.map(addressStrs, function(a) {
        return Utils.translateAddress(a);
      });
      next();
    },
    function(next) {
...
WalletService.prototype.getSendMaxInfo = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    var feeLevels = Defaults.FEE_LEVELS;
    if (opts.feeLevel) {
      if (!_.any(feeLevels, {
        name: opts.feeLevel
      }))
...
WalletService.prototype._sampleFeeLevels = function(network, points, cb) {
  ...
  self._getBlockchainExplorer(network, function(err, bc) {
    if (err || !bc) return cb(new Error('Could not get blockchain explorer instance'));
...
WalletService.prototype.getFeeLevels = function(opts, cb) {
  ...
  checkAndUseFeeLevelsCache(function(err, values) {
    ...
    self._sampleFeeLevels(opts.network, samplePoints(), function(err, feeSamples) {
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    ...
    self.logw('Error building Digibytejs transaction', ex);
    return ex;
  }

  if (bitcoreError instanceof digibyte.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (bitcoreError instanceof digibyte.errors.Transaction.DustOutputs)
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, digibyte.Transaction.DUST_AMOUNT);
...
WalletService.prototype._broadcastRawTx = function(network, raw, cb) {
  this._getBlockchainExplorer(network, function(err, bc) {
    if (err || !bc) return cb(new Error('Could not get blockchain explorer instance'));
 ...
 * @param {Object} opts
 * @param {string} [opts.network = 'livenet'] - The Digibyte network for this transaction.
...
WalletService.prototype.broadcastRawTx = function(opts, cb) {
  ...
  self._broadcastRawTx(opts.network, opts.rawTx, cb);
};


WalletService.prototype._checkTxInBlockchain = function(txp, cb) {
  if (!txp.txid) return cb();
  this._getBlockchainExplorer(txp.network, function(err, bc) {
...
WalletService.prototype.broadcastTx = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      self._broadcastRawTx(wallet.network, raw, function(err, txid) {
        if (err) {
  ...
  function getNormalizedTxs(wallet, addresses, from, to, cb) {
    ...
    async.series([
      ...
      function(next) {
        ...
        self._getBlockchainExplorer(wallet.network, function(err, bc) {
      ...
      function(next) {
        ...
        self._getBlockchainHeight(wallet.network, function(err, height) {
