...
var crypto = digibyte.crypto;
var encoding = digibyte.encoding;
...
Utils.hashMessage = function(text, noReverse) {
  ...
  if (!noReverse) {
    ret = new digibyte.encoding.BufferReader(ret).readReverse();
  }
  return ret;
};
...
Utils.formatAmountInBtc = function(amount) {
  return Utils.formatAmount(amount, 'dgb', {
    minDecimals: 8,
    maxDecimals: 8,
  }) + 'dgb';
};
...
Utils.parseVersion = function(version) {
  ...
  v.agent = _.contains(['dwc', 'dws'], x[0]) ? 'bwc' : x[0];
  x = x[1].split('.');
...
Utils.translateAddress = function(address, coin) {
  var origAddress = new digibyte.Address(address);
  var origObj = origAddress.toObject();
  console.log(origObj);
 // var result = digibyte.Address.fromObject(origObj)
  return origObj.toString();
};
